# Todo

- Fix smoke sprite rotation issue where it doesn't always face player as they look left or right.
- Shot camera shake shouldn't move gun back or camera forward as much because it causes thhe player to see further into the scope past the lens
- Add gun model sway when looking around which should be reduced in intensity when aimed down sight.
- Add realistic shot tracer ( should look like a line of smoke ).
- Add basic map with things to jump off of.
- Add jump and sprint.
- Add fov, mouse sensitivity, and keybind settings.
- Add modern looking font for ui if needed.

## Sound

- Update fps Sniper shot sound.
- Reload sound.
- Rechamber sound.
